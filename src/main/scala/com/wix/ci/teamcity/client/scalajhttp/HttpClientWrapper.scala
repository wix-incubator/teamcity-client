...
class HttpClientWrapper(username: String, password: String, timeout: Int = 5000) extends HttpClient {
  ...
  override def executePutPlainText(url: String, body: String, accept: String): String = {
  ...
  private def execute(url: String,
                      ...
                      accept: String = "application/json"): String = {
  ...
  private def checkHttpStatusCode(code: Int, msg: String) = {
    if (code != 200 && code != 201 && code != 204)
      throw new TeamcityServerException(code, s"Error from server status code $code $msg")
  }
}
